...
import util.UUIDTokenGenerator;
 ...
 */
public class SimpleUserDao implements UserDao,AutoCloseable{
    private ConnectionUtil connectionUtil;
    private PropertiesHandler propertiesHandler;
    private Connection con;
    private final Logger LOG = LoggerFactory.getLogger(SimpleUserDao.class);
    @Autowired
    public SimpleUserDao(ConnectionUtil connectionUtil, PropertiesHandler propertiesHandler) {
    ...
    public boolean setToken(String userName) {
        String token = UUIDTokenGenerator.createToken();
        try (PreparedStatement ps = con.prepareStatement(
    ...
    public boolean deleteToken(String userName) {
        try (PreparedStatement ps = con.prepareStatement(
                "UPDATE Users SET token = IF(loggedIn = 1, NULL, token), " +
                        "loggedIn = IF(loggedIn = 1, 0, loggedIn - 1) WHERE username = ?")) {
            ps.setString(1, userName);
            int success = ps.executeUpdate();
            if (success > 0) {
