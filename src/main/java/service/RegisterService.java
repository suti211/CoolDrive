...
@Path("/register")
public class RegisterService extends SimpleControllersFactory {
	...
	public Status getUser(User input,@Context HttpServletRequest request) {
		try (UserFileController userFileController = getUserFileController();
			...
			if (userID == -1) {
				...
				int userHomeId = userFileController.addNewUserFile(new UserFile(PropertiesHandler.Paths.ROOTPATH, 0, input.getUserName(), "dir", startQuantity, true, userId, parentId, "Homefolder"));
